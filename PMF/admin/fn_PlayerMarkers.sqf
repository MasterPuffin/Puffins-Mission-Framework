...
*/
[]spawn{
  if (PMF_var_PlayerMarkers isEqualTo 1) exitWith {
      PMF_var_PlayerMarkers = 0;
      onEachFrame {}; {
  ...
  PMF_var_PlayerMarkers = 1;
  markerList = [];
  markerUnits = [];

  hint "Markers enabled - Check map!";
  while {true} do {
      if (PMF_var_PlayerMarkers isEqualTo 0) exitWith {};

      {
          ...
          if (side _x isEqualTo side player) then //Creates marker for people on your team (shows up blue)
              {
      ...
      sleep 1;
      if (PMF_var_PlayerMarkers isEqualTo 0) exitWith {}; //checks if the marker hack is disabled
      {
          _x setMarkerPosLocal getPos(markerUnits select(markerList find _mName));
          _x setMarkerDirLocal getDir(markerUnits select(markerList find _mName));
      }
      forEach markerList; //updates position and direction
      sleep 1;
      if (PMF_var_PlayerMarkers isEqualTo 0) exitWith {}; {
          _x setMarkerPosLocal getPos(markerUnits select(markerList find _mName));
          _x setMarkerDirLocal getDir(markerUnits select(markerList find _mName));
      ...
      forEach markerList; //clears markers to allow them to be refreshed when it goes back to the beginning of the loop
      markerUnits = [];
      markerList = [];
  };
};
